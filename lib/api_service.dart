...
class ApiService {
  ...
  /// accepted (or backed up on). Backed by its own endpoint
  /// (GET /responder/incidents/history, IncidentController::
  /// resolvedForResponder) rather than filtering getAssignedIncidents()'s
  /// result, because that endpoint excludes resolved incidents entirely
  /// by design — a resolved incident simply disappears from it the
  /// moment it's closed out.
  static Future<ApiResponse> getResolvedIncidents() async {
    try {
      final headers = await _responderAuthHeaders();
      final response = await http
          .get(
            Uri.parse('$baseUrl/responder/incidents/history'),
            headers: headers,
          )
          .timeout(const Duration(seconds: 15));
      ...
      return ApiResponse.error(
        'Failed to load report history (${response.statusCode}): $detail',
      );
    } catch (e) {
      return ApiResponse.error(_handleError(e));
    }
